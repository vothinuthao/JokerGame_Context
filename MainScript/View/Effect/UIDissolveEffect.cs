...
namespace Coffee.UIExtensions
{
...
#endif
	{
		...
		[SerializeField] [Range(0, 1)] float m_Location = 0f;
		...
		{
			DOTween.To(() => location, x =>  location = x, 0f, m_duration /2f)
		...
		{
			DOTween.To(() => location, x =>  location = x, 1f, m_duration)
				.SetEase(Ease.OutQuad)
				.OnComplete(() => {
		...
		public float location
		{
			...
			} set { m_Location = Mathf.Clamp(value, 0, 1); _SetDirty(); }
		}
